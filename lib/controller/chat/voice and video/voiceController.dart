...
const String appId =
    "977621c3bde5423ebb3782aaf5b6edbc"; //e151cc863dd34adc9f76f085e4fb7b78
...
class VoiceController extends GetxController {
  ...
  late int _remoteUid; // uid of the remote user
  late bool _isJoined =
      false; // Indicates if the local user has joined the channel
  // late RtcEngine agoraEngine; // Agora engine instance
  late bool micWidget = false;
  late bool muteMic = false;
  late bool isJoined = true;
  late bool inCall = false;
  late int currentTime = 0;
  late Timer timer;
  late String talkStatus;
  late String memberTalkTime;
  late String adminTalkTime;
  late String superAdminTalkTime;
  ...
  Future<void> setupVoiceSDKEngine() async {
    ...
    //       if (remoteUid != null) {
    //         _remoteUid = 0; // تحقق من أن remoteUid غير null قبل التعيين
    //         update();
    //       }
    //     },
    //   ),
  ...
  Future<void> leave() async {
    ...
    _remoteUid = 0;
    var url =
        Uri.parse('https://lametnachat.com/messages/deleteMembersInCall.php');
    var response = await http.post(url, body: {
      "roomId": Get.arguments["room_id"],
      "name": userName,
      // "joinOrLeave": "9", //left 1 joined 0
