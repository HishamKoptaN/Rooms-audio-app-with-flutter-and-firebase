...
final userControllerProvider = StateNotifierProvider<UserController, bool>(
    (ref) => UserController(
        userRepository: ref.watch(userRepoController),
        ref: ref,
...
class UserController extends StateNotifier<bool> {
  ...
  Future<bool> editUser(
    ...
    try {
      ...
      String profileImg = "";
      if (imageData != null) {
        profileImg = await storageRepository.storeFile(
            path: 'SMusers/${uid}', imageData: imageData);
      }
      bool status = await userRepository.editUser(
          uid: uid, bio: bio, name: name, profileImg: profileImg);
      state = false;
      return status;
    } catch (e) {
      state = false;
