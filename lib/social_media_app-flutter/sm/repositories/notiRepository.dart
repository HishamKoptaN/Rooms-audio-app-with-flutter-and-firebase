...
class NotiRepository {
  ...
  CollectionReference get _notiMessageCollection =>
      firestore.collection(Constants.noteMessageCollection);
  ...
  Stream<List<NotiModel>> getAllNotiofUser({required String currentUserId}) {
    return _notiMessageCollection
        .where('userId', isEqualTo: currentUserId)
        .orderBy('dateTime', descending: true)
        .snapshots()
        .map((event) {
      return event.docs
          .map((e) => NotiModel.fromMap(e.data() as Map<String, dynamic>))
          .toList();
    });
  }
