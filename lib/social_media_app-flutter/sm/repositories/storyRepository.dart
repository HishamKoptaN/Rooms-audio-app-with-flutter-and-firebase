...
class StoryRepository {
  ...
  Future<bool> addNewStory(Uint8List file, UserModel user) async {
    try {
      ...
      String storyImg = await storageRepository.storeFile(
          path: 'SMstories/${user.uid}/${storyId}', imageData: file);
  ...
  Future<List<UserModel>> getUsersWhoSharedStories(
      List<String> followingIds) async {
    Set<String> uniqueUserIds = Set<String>();

    List<UserModel> userList = [];

    QuerySnapshot querySnapshot = await _storyCollection
        .where(
          'createdAt',
          isGreaterThan: DateTime.now()
              .subtract(const Duration(hours: 24))
    ...
    for (DocumentSnapshot document in querySnapshot.docs) {
      StoryModel story =
          StoryModel.fromMap(document.data() as Map<String, dynamic>);
